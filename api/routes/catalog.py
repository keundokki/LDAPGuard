...
from api.models.models import Backup, BackupStatus, LDAPServer, User
...
async def get_catalog_stats(
    ...
    for backup in all_backups:
        status_value = backup.status.value if hasattr(
            backup.status, 'value') else str(backup.status)
        backups_by_status[status_value] = backups_by_status.get(status_value, 0) + 1

    # Group by type
    backups_by_type = {}
    for backup in all_backups:
        type_value = backup.backup_type.value if hasattr(
            backup.backup_type, 'value') else str(
            backup.backup_type)
    ...
    backups_by_category = {}
    for backup in all_backups:
        category_value = backup.category.value if hasattr(
            backup.category, 'value') else str(backup.category)
        backups_by_category[category_value] = backups_by_category.get(
            category_value, 0) + 1
    ...
    verified_backups = sum(
        1 for b in all_backups if b.verification_status == "verified")
...
async def export_backups(
    ...
    created_after: Optional[datetime] = Query(None, description="Filter by creation date"),  # noqa: E501
    created_before: Optional[datetime] = Query(None, description="Filter by creation date"),  # noqa: E501
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    for backup in backups:
        writer.writerow(
            [
                ...
                backup.backup_type.value if hasattr(
                    backup.backup_type,
                    'value') else str(
                    backup.backup_type),
                backup.category.value if hasattr(
                    backup.category,
                    'value') else str(
                    backup.category),
                backup.status.value if hasattr(
                    backup.status,
                    'value') else str(
                    backup.status),
        ...
        data.append(
            {
                ...
                "backup_type": backup.backup_type.value if hasattr(
                    backup.backup_type,
                    'value') else str(
                    backup.backup_type),
                "category": backup.category.value if hasattr(
                    backup.category,
                    'value') else str(
                    backup.category),
                "status": backup.status.value if hasattr(
                    backup.status,
                    'value') else str(
                    backup.status),
                "file_size": backup.file_size,
                "entry_count": backup.entry_count,
                "created_at": backup.created_at.isoformat() if backup.created_at else None,  # noqa: E501
                "completed_at": backup.completed_at.isoformat() if backup.completed_at else None,  # noqa: E501
