...
async def upload_backup_to_cloud(
    ...
    """
    # Check permissions
    if current_user.role not in [UserRole.OPERATOR, UserRole.BACKUP_ADMIN, UserRole.SECURITY_ADMIN, UserRole.ADMIN]:
        raise HTTPException(
    ...
    if backup.cloud_uploaded:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Backup {backup_id} is already uploaded to cloud storage at {backup.cloud_storage_path}"
        )
    ...
    try:
        ...
        return CloudStorageUploadResponse(
            backup_id=backup_id,
            success=True,
            message=f"Backup uploaded successfully to {upload_result['provider']} storage",
...
async def download_backup_from_cloud(
    ...
    try:
        ...
        # Download from S3
        logger.info(f"Downloading backup {backup_id} from cloud storage: {backup.cloud_storage_path}")
        ...
        return {
            "success": True,
            "message": f"Backup downloaded successfully from {backup.cloud_provider} storage",
...
async def delete_backup_from_cloud(
    ...
    try:
        # Delete from S3
        logger.info(f"Deleting backup {backup_id} from cloud storage: {backup.cloud_storage_path}")
...
async def get_backup_cloud_info(
    ...
    try:
        obj_info = await storage_service.get_object_info(backup.cloud_storage_path, db=db)
