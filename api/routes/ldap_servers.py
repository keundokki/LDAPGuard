...
from api.schemas.schemas import LDAPServerCreate, LDAPServerUpdate, LDAPServerResponse
...
async def get_ldap_server(server_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not server:
        raise HTTPException(
            ...
            detail="LDAP server not found"
        )
    
    return server


@router.post("/", response_model=LDAPServerResponse, status_code=status.HTTP_201_CREATED)
async def create_ldap_server(server_data: LDAPServerCreate, db: AsyncSession = Depends(get_db)):
    """Create a new LDAP server configuration."""
    # Check if name exists
    result = await db.execute(select(LDAPServer).where(LDAPServer.name == server_data.name))
