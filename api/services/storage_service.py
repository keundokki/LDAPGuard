...
class StorageService:
    ...
    def __init__(self):
        """Initialize storage service (S3 client created dynamically with latest settings)."""
    ...
    async def _get_s3_client(self, db: AsyncSession):
        ...
        self.storage_class = db_config.get('s3_storage_class') or settings.S3_STORAGE_CLASS
    ...
    async def upload_backup(
        ...
        logger.info(f"Uploading backup to S3: {object_key} ({file_size} bytes, class: {storage_class})")
    ...
    async def download_backup(
        ...
        try:
            ...
            logger.info(f"Successfully downloaded backup from S3: {object_key} ({file_size} bytes)")
            return result
    ...
    async def delete_backup(self, object_key: str, db: Optional[AsyncSession] = None) -> Dict[str, Any]:
        """
    ...
    async def get_object_info(self, object_key: str, db: Optional[AsyncSession] = None) -> Dict[str, Any]:
        """
