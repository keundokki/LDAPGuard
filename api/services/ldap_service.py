...
class LDAPService:
    ...
    def __init__(self, host: str, port: int, use_ssl: bool, base_dn: str, 
    ...
    def search_all_entries(self, search_filter: str = "(objectClass=*)") -> List[Tuple[str, Dict]]:
    ...
    def backup_to_ldif(self, output_path: str, search_filter: str = "(objectClass=*)") -> int:
    ...
    def backup_to_json(self, output_path: str, search_filter: str = "(objectClass=*)") -> int:
                    ...
                    if current_dn and current_attrs:
                        try:
                            ...
                            for attr, values in current_attrs.items():
                                ldif_attrs[attr] = [v.encode('utf-8') if isinstance(v, str) else v for v in values]
                            
                            # Add entry
                            self.conn.add_s(current_dn, modlist.addModlist(ldif_attrs))
                            restored_count += 1
    ...
    def get_modified_entries(self, since: datetime, search_filter: str = "(objectClass=*)") -> List[Tuple[str, Dict]]:
