...
class TestLDAPServerRoutes:
    ...
    def test_test_ldap_connection(self, client, test_ldap_server_data):
        """Test LDAP connection test endpoint."""
        response = client.post("/ldap/test", json=test_ldap_server_data)
        assert response.status_code in [
            status.HTTP_200_OK,
            status.HTTP_400_BAD_REQUEST,  # Connection may fail
            status.HTTP_401_UNAUTHORIZED  # May require authentication
        ]
